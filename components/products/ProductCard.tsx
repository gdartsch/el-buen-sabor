...
export const ProductCard: FC<Props> = ({ product }) => {
  ...
  for (let i = 0; i < product.recipe.length; i++) {
    const { ingredient } = useIngredients(
      `/ingredients?nombre=${product.recipe[i][0]}`
    )
  ...
  const stock = (): number => {
    ...
    let stockedIngredients = ingredients!.map(
    ...
    for (let i = 0; i < neededIngredients.length; i++) {
      stock.push(stockedIngredients[i] / neededIngredients[i][1])
    }

    if (neededIngredients.length > stockedIngredients.length) {
      return 0
    }
